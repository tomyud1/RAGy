...
const BATCH_SIZE = 20;
const VECTOR_DB_DIR = path.join(process.cwd(), 'data/vector_db_mpnet');
...
class MPNetEmbeddingGenerator {
  ...
  async initialize() {
    console.log('\n╔═══════════════════════════════════════════════════════════════════╗');
    console.log('║                                                                   ║');
    console.log('║         🤖  ALL-MPNET-BASE-V2 EMBEDDING GENERATOR  🤖            ║');
    console.log('║                                                                   ║');
    console.log('╚═══════════════════════════════════════════════════════════════════╝\n');
  ...
  async run() {
    try {
      ...
      console.log('═'.repeat(80));
      console.log('🎉 ALL-MPNET-BASE-V2 EMBEDDING GENERATION COMPLETE!\n');
      console.log('Next steps:');
      console.log('  1. Test: npm run server (then open http://localhost:3001/rag-tester.html)');
      console.log('  2. Compare all models in the RAG tester UI');
      console.log('═'.repeat(80) + '\n');
...
const generator = new MPNetEmbeddingGenerator();
generator.run();
